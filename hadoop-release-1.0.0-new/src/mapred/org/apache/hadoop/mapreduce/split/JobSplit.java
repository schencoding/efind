...
public class JobSplit {
  ...
  //BEGIN--Added by John at 2012-05-02
    private boolean bRunAtIndex=false;
    private int indexId;
    ...
    public SplitMetaInfo(String[] locations, long startOffset, 
            long inputDataLength, boolean bRunAtIndex, int indexId, String[] indexLocations) {
          this.locations = locations;
          this.startOffset = startOffset;
          this.inputDataLength = inputDataLength;
          this.bRunAtIndex = bRunAtIndex;
    ...
    public void readFields(DataInput in) throws IOException {
      ...
      bRunAtIndex = Boolean.parseBoolean(str);
			if (bRunAtIndex) {
				indexId = WritableUtils.readVInt(in);
				int lenOfIndex = WritableUtils.readVInt(in);
				indexLocations = new String[lenOfIndex];
				for (int i = 0; i < indexLocations.length; i++) {
					indexLocations[i] = Text.readString(in);
				}
			}
    }
    ...
    public void write(DataOutput out) throws IOException {
      ...
      Text.writeString(out, Boolean.toString(bRunAtIndex));
			if (bRunAtIndex) {
				WritableUtils.writeVInt(out, indexId);
				WritableUtils.writeVInt(out, indexLocations.length);
				for (int i = 0; i < indexLocations.length; i++) {
					Text.writeString(out, indexLocations[i]);
				}
			}
    ...
    public String toString() {
      ...
      buf.append("run@index : " + bRunAtIndex + "\n");
      if(bRunAtIndex){
      buf.append("index-id : " + indexId + "\n");
      if(indexLocations != null){
	    	  for(String indexLoc : indexLocations){
	    		  buf.append(" " + indexLoc + "\n");
	    	  }
	      }
      }
  ...
  //BEGIN--the following function is added by John at 2012-05-02
    public boolean checkIfRunAtIndex(){
    	return this.bRunAtIndex;
    }
    
    public void setIfRunAtIndex(boolean value){
    	this.bRunAtIndex = value;
    }
    ...
    public String[] getIndexLocations(){
    	return this.indexLocations;
    }
    
    public void setIndexLocations(String[] values){
    	this.indexLocations = values;
    }
  ...
  public static class TaskSplitMetaInfo {
	...
	public String toString() {
		String resultStr = "";
		resultStr += "SPLIT: " + this.getSplitIndex().splitLocation + ", " + this.getSplitIndex().getStartOffset() + "\n";
		resultStr += "Length: " + this.getInputDataLength() + "\n";
		resultStr += "Split Locations: ";
		for(int i=0; i< this.getLocations().length; i++){
			resultStr += this.locations[i] + ", ";
		}
			resultStr += "Index ID: " + this.indexId + "\n";
			resultStr += "Index Locations: ";
			if (indexLocations != null) {
				for (int i = 0; i < this.indexLocations.length; i++) {
					resultStr += this.indexLocations[i] + ", ";
				}
			}
		resultStr += "\n";
		return resultStr;
