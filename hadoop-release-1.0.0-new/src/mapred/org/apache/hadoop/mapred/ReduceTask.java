...
class ReduceTask extends Task {
  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    private class MapOutputLocation {
      ...
      public void setJobIdStr(String str){
    	  this.jobIdStr = str;
      }
    }
    ...
    private class MapOutputCopier extends Thread {
      ...
      private SecretKey rebuildSecretkey(String jobId) throws IOException{
    	  Token<JobTokenIdentifier> jt = new Token<JobTokenIdentifier>();
    	  Configuration hdfsConf = new Configuration();
  		FileSystem hdfs = FileSystem.get(hdfsConf);
  		Path hdfsFile = new Path(JobInProgress.TOKEN_STORE_PATH_PREFIX + jobId+"_token");
      ...
      private InputStream setupSecureConnection(MapOutputLocation mapOutputLoc, 
    	  ...
    	  if(mapOutputLoc.getJobIdStr()==null){
        ...
        return input;
					...
					String jobIdStr = mapOutputLoc.getJobIdStr();
					SecretKey jobSecretKey = rebuildSecretkey(jobIdStr);
					
					// generate hash of the url
					String msgToEncode = SecureShuffleUtils.buildMsgFrom(connection.getURL());
					String encHash = SecureShuffleUtils.hashFromString(msgToEncode, jobSecretKey);
					...
					// get the replyHash which is HMac of the encHash we sent to
					// the server
					...
					// verify that replyHash is HMac of encHash
					SecureShuffleUtils.verifyReply(replyHash, encHash, jobSecretKey);
					if (LOG.isDebugEnabled())
						LOG.debug("for url=" + msgToEncode + " sent hash and receievd reply");
					return input;
				}
    ...
    private String[] loadSkippedMaps() {  	 
  	  String str = conf.get(SKIPPED_MAP_INFO);
  	  if(str!=null)
  		  return str.split(",");
  	  else 
  		  return null;
		...
		public void addSkippedMaps() throws MalformedURLException {
			String strbSkipMapsInReduce = conf.get(JobInProgress.B_SKIP_MAP_IN_REDUCE);
		    boolean bSkipMapsInReduce = false;
		    if(strbSkipMapsInReduce != null){
		    	bSkipMapsInReduce = Boolean.parseBoolean(strbSkipMapsInReduce);
		    }
		    
		    String strbAppendMapsInReduce = conf.get(JobInProgress.B_APPEND_MAP_IN_REDUCE);
			...
			if (!bSkipMapsInReduce || bAppendMapsInReduce) {
				List<Integer> skippedSplits = JobInProgress.getSkippedSplits(conf);
				String[] parts = loadSkippedMaps();
				if (parts != null) {
					for (int i = 0; i < skippedSplits.size(); i++) {
						String taskTrackHttpStr = parts[2 * i];
						String taskAttemptIdStr = parts[2 * i + 1];

						URI u = URI.create(taskTrackHttpStr);
						String host = u.getHost();
						TaskAttemptID taskId = TaskAttemptID.forName(taskAttemptIdStr);
						String jobIdStr = taskId.getJobID().toString();

						URL mapOutputLocation = new URL(taskTrackHttpStr + "/mapOutput?job=" + taskId.getJobID()
								+ "&map=" + taskId + "&reduce=" + getPartition());
						
						LOG.info("Append map: " + mapOutputLocation.getPath());
						System.out.println("Append map: " + mapOutputLocation.getPath());
						...
						MapOutputLocation mapOutputLoc = new MapOutputLocation(taskId, host, mapOutputLocation);
						mapOutputLoc.setJobIdStr(jobIdStr);
						loc.add(mapOutputLoc);
					}
				}
			}
    ...
    public boolean fetchOutputs() throws IOException {
      ...
      for (int i=0; i < numCopiers; i++) {
        ...
        LOG.info("reduceTask.getJobTokenSecret() type:" + reduceTask.getJobTokenSecret().getClass());
        LOG.info("reduceTask.getJobTokenSecret(): " + reduceTask.getJobTokenSecret().getEncoded());
        copiers.add(copier);
        copier.start();
      }
      ...
      //Added by John
      //Add the skipped map to mapLocations
              ...
              URL mapOutputLocation = new URL(event.getTaskTrackerHttp() + 
                      ...
                      "&reduce=" + getPartition());
						/*if (getPartition() == 0) {
				            ...
				            //          "/mapOutput?job=" + taskId.getJobID() +
				            //          "&map=" + "attempt_201205080314_0001_m_000000_0" + 
				            //          "&reduce=" + getPartition());
							mapOutputLocation = new URL(
									"http://hplcchina-HP-xw8600-Workstation:50060/mapOutput?job=job_201205080314_0001&map=attempt_201205080314_0001_m_000000_0&reduce=0");
							LOG.info("Ingested map output: " + mapOutputLocation.toString());
						}*/
