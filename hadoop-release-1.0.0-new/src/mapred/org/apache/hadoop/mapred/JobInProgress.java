...
public class JobInProgress {
	public static final String SPLITS_TO_SKIP = "mapred.splitstoskip";
	public static final String SKIPPED_MAP_INFO="ReduceTask.skippedmaps.info";
	public static final String B_SKIP_MAP_IN_MAP = "Map.b.skip.map.inmap";
	public static final String B_SKIP_MAP_IN_REDUCE = "ReduceTask.b.skipmap.inreduce";
	public static final String B_APPEND_MAP_IN_REDUCE = "ReduceTask.b.appendmap.inreduce";
	public static final String REDUCE_AND_INDEX_MAPPING = "JobInProgress.reduce.index.mapping";
	public static final String B_RUN_REDUCE_AT_INDEX = "mapred.b.run.reduce.at.index";
	public static final String RUN_REDUCE_AT_INDEX = "mapred.run.reduce.at.index";
	
	
	
	public static final String SKIPPED_REDUCE = "mapred.skipped.reduce";
	public static final String TOKEN_STORE_PATH_PREFIX = "";
  ...
  Map<Node, List<TaskInProgress>> nonRunningReduceCache;
    ...
    try {
      ...
      if (ts != null) {
        for (Token<? extends TokenIdentifier> token : ts.getAllTokens()) {
          ...
          LOG.info("userUGI.addToken(token): " + token.getIdentifier() + " " + token.getPassword());
        }
      }
  ...
  private Map<Node, List<TaskInProgress>> createReduceCache()throws UnknownHostException{
		Map<Node, List<TaskInProgress>> reduceCache = new IdentityHashMap<Node, List<TaskInProgress>>(maxLevel);

		HashMap<Integer, List<String>> partitionIndexMapping = null;
		String partitionIndexMappingStr = conf.get(this.REDUCE_AND_INDEX_MAPPING);
		LOG.info(REDUCE_AND_INDEX_MAPPING + ": " + partitionIndexMappingStr);
		if (partitionIndexMappingStr != null) {
			partitionIndexMapping = new HashMap<Integer, List<String>>();
			String[] lines = partitionIndexMappingStr.split("\n");
			for (int i = 0; i < lines.length; i++) {
				...
				if (strs.length > 1) {
					...
					partitionIndexMapping.put(partition, list);
				}
			}
		}
		if (partitionIndexMapping != null) {
			for (int i = 0; i < numReduceTasks; i++) {
				TaskInProgress tip = reduces[i];
				List<String> hostList = partitionIndexMapping.get(tip.getIdWithinJob());
				if (hostList != null) {
					for (String host : hostList) {
						LOG.info("host:" + host);
						Node node = jobtracker.resolveAndAddToTopology(host);
						LOG.info("tip:" + reduces[i].getTIPId() + " has INDEX partition on node:" + node);
						for (int j = 0; j < maxLevel; j++) {
							List<TaskInProgress> hostReduces = reduceCache.get(node);
							if (hostReduces == null) {
								hostReduces = new ArrayList<TaskInProgress>();
								reduceCache.put(node, hostReduces);
								hostReduces.add(reduces[i]);
							}

							if (hostReduces.get(hostReduces.size() - 1) != reduces[i]) {
								hostReduces.add(reduces[i]);
							}
							node = node.getParent();
						}
  ...
  private Map<Node, List<TaskInProgress>> createCache(
                                 TaskSplitMetaInfo[] splits, int maxLevel, List<Integer> skippedSplits)
                                 throws UnknownHostException {
    Map<Node, List<TaskInProgress>> cache = 
      new IdentityHashMap<Node, List<TaskInProgress>>(maxLevel);
    
    String strbSkipMapInMap = conf.get(B_SKIP_MAP_IN_MAP);
    boolean bSkipMapInMap = false;
    if(strbSkipMapInMap != null){
    	bSkipMapInMap = Boolean.parseBoolean(strbSkipMapInMap);    	
    }
    int mapCounter = -1;
    Set<String> uniqueHosts = new TreeSet<String>();
    for (int i = 0; i < splits.length; i++) {
    	if(bSkipMapInMap && skippedSplits.contains(i)){
    		continue;
    	}
    	mapCounter ++;
      String[] splitLocations = splits[i].getLocations();
      if (splitLocations == null || splitLocations.length == 0) {
        nonLocalMaps.add(maps[mapCounter]);
        continue;
      }
      
      if(splits[i].checkIfRunAtIndex()){
    	  String[] indexLocations = splits[i].getIndexLocations();
    	  for(String host: indexLocations) {
    	        Node node = jobtracker.resolveAndAddToTopology(host);
    	        LOG.info("Index host: " + host);
    	        LOG.info("Index host node: " + node);
    	        uniqueHosts.add(host);
    	        LOG.info("tip:" + maps[mapCounter].getTIPId() + " has INDEX split on node:" + node);
    	        for (int j = 0; j < maxLevel; j++) {
    	          List<TaskInProgress> hostMaps = cache.get(node);
    	          if (hostMaps == null) {
    	            hostMaps = new ArrayList<TaskInProgress>();
    	            cache.put(node, hostMaps);
    	            hostMaps.add(maps[mapCounter]);
    	          }
    	          ...
    	          if (hostMaps.get(hostMaps.size() - 1) != maps[mapCounter]) {
    	            hostMaps.add(maps[mapCounter]);
    	          }
    	          node = node.getParent();
    	        }
				...
				for (String host : splitLocations) {
					...
					if (node != null) {
						uniqueHosts.add(host);
						LOG.info("tip:" + maps[mapCounter].getTIPId() + " has split on node:" + node);
						for (int j = 0; j < maxLevel; j++) {
							List<TaskInProgress> hostMaps = cache.get(node);
							if (hostMaps == null) {
								hostMaps = new ArrayList<TaskInProgress>();
							...
							// check whether the hostMaps already contains an
							// entry
							// for a TIP
							// This will be true for nodes that are racks and
							// multiple nodes in
							// the rack contain the input for a tip. Note that
							// if it
							// already
							// exists in the hostMaps, it must be the last
							// element
							// there since
  ...
  throws IOException, KillInterruptedException, UnknownHostException {
    ...
    //Added by John
    List<Integer> skippedSplits = getSkippedSplits(conf);
    int numSkippedSplits = skippedSplits.size();
    List<Integer> skippedReduces = getSkippedReduces(conf);
    int numSkippedReduces = skippedReduces.size();
    String strbSkipMapsInReduce = conf.get(this.B_SKIP_MAP_IN_REDUCE);
    boolean bSkipMapsInReduce = false;
    if(strbSkipMapsInReduce != null){
    	bSkipMapsInReduce = Boolean.parseBoolean(strbSkipMapsInReduce);
    }
    
    String strbAppendMapsInReduce = conf.get(this.B_APPEND_MAP_IN_REDUCE);
    boolean bAppendMapsInReduce = false;
    if(strbAppendMapsInReduce != null){
    	bAppendMapsInReduce = Boolean.parseBoolean(strbAppendMapsInReduce);
    }
    ...
    int assumedNumReduceTasks = numReduceTasks;
    numReduceTasks -= numSkippedReduces;
    ...
    numMapTasks = splits.length;
    if(bSkipMapInMap){
    	numMapTasks = splits.length - numSkippedSplits;
    }
    LOG.info("# of skipped splits: " + numSkippedSplits);
    LOG.info("Skipped splits: " + conf.get("mapred.splitstoskip"));
    
    LOG.info("# of skipped reduces: " + numSkippedReduces);
    LOG.info("Skipped reduces: " + conf.get(this.SKIPPED_REDUCE));
    
    int assumedNumMapTasks = splits.length;    
    //int assumedNumMapTasks = numMapTasks;
    
    if(bSkipMapsInReduce){
    	assumedNumMapTasks = numMapTasks;
    }
    
    if(bAppendMapsInReduce){
    	assumedNumMapTasks += numSkippedSplits;
    }
    
  //this method is added by John at 2012-05-02
    logTaskSplitMetaInfo(splits);
    ...
    for(int i=0; i < splits.length; ++i) {
      ...
      maps[j] = new TaskInProgress(jobId, jobFile, 
                                   splits[i], 
                                   jobtracker, conf, this, j, numSlotsPerMap);
      LOG.info("map " + j + " using split " + i);
    ...
    if (numMapTasks > 0) { 
      nonRunningMapCache = createCache(splits, maxLevel, skippedSplits);
    }
    ...
    int reduceCounter = -1;
    for (int i = 0; i < assumedNumReduceTasks; i++) {
    	if(skippedReduces.contains(i)){
    		continue;
    	}
    	reduceCounter ++;
      reduces[reduceCounter] = new TaskInProgress(jobId, jobFile, 
    		  						assumedNumMapTasks, i, 
                                      jobtracker, conf, this, numSlotsPerReduce);
      reduces[reduceCounter].setIdInReduceTasks(reduceCounter);
      nonRunningReduces.add(reduces[reduceCounter]);
    }
    
    nonRunningReduceCache = this.createReduceCache();
    ...
    // cleanup reduce tip.
    cleanup[1] = new TaskInProgress(jobId, jobFile, assumedNumMapTasks,
    ...
    // setup reduce tip.
    setup[1] = new TaskInProgress(jobId, jobFile, assumedNumMapTasks,
   ...
   LOG.info("Job " + jobId + " initialized successfully with " + numMapTasks
	...
	public static List<Integer> getSkippedSplits(JobConf conf) {
		List<Integer> list = new ArrayList<Integer>();
		String str = conf.get(SPLITS_TO_SKIP);
		if (str != null) {
	...
	public static List<Integer> getSkippedReduces(JobConf conf){
		List<Integer> list = new ArrayList<Integer>();
		String str = conf.get(SKIPPED_REDUCE);
		if (str != null) {
			String[] parts = str.split(",");
			int length = parts.length;
			for (int i = 0; i < length; i++) {
  ...
  //this method is added by John at 2012-05-02
  void logTaskSplitMetaInfo(TaskSplitMetaInfo[] splits){
	  for(int i=0; i< splits.length; i++){
		  TaskSplitMetaInfo tsmi = splits[i];
		  LOG.info(tsmi.toString());
	  }
  }
  ...
  private synchronized void failReduce(TaskInProgress tip) {
	...
	private synchronized TaskInProgress findReduceTaskFromList(Collection<TaskInProgress> tips,
			TaskTrackerStatus tts, int numUniqueHosts, boolean removeFailedTip) {
		String reduceIndexMappingStr = conf.get(this.REDUCE_AND_INDEX_MAPPING);
		if (reduceIndexMappingStr != null) {
			...
			if (node != null) {
				...
				Node key = node;
				int level = 0;
				int maxLevelToSchedule = Math.min(anyCacheLevel, maxLevel);
				for (level = 0; level < maxLevelToSchedule; ++level) {
					List<TaskInProgress> cacheForLevel = nonRunningReduceCache.get(key);
					if (cacheForLevel != null) {
						...
						if (tip != null) {
							...
							if (cacheForLevel.size() == 0) {
								nonRunningReduceCache.remove(key);
							}
							LOG.info("Find a new reduce from local node to the root [bottom up cache lookup]." + tip.getTIPId());
							return tip;
						}
					}
					key = key.getParent();
			...
			// 2. Search breadth-wise across parents at max level for
			// non-running
			...
			for (Node parent : nodesAtMaxLevel) {
				...
				List<TaskInProgress> cache = nonRunningReduceCache.get(parent);
				if (cache != null) {
					...
					if (tip != null) {
						...
						if (cache.size() == 0) {
							nonRunningReduceCache.remove(parent);
						}
			...
			TaskInProgress rtTip = findTaskFromList(tips, tts, numUniqueHosts, removeFailedTip);
			if(rtTip != null )
				LOG.info("Choosing a non-local task " + rtTip.getTIPId());
			return rtTip;
  ...
  private synchronized int findNewMapTask(final TaskTrackerStatus tts, 
    ...
    LOG.info("Find map for tracker: " + tts.getTrackerName());
    LOG.info("tts.getHost(): " + tts.getHost());
    LOG.info("Node node = jobtracker.getNode(tts.getHost()) nodeName:" + node);
      ...
      for (level = 0;level < maxLevelToSchedule; ++level) {
        ...
        if (cacheForLevel != null) {
          ...
          if (tip != null) {
            ...
            //Added by John
            if(level ==0){
            	String[] splitLocs = tip.getSplitLocations();
            	String strSplitLocs = "";
            	for(int i=0; i<splitLocs.length; i++){
            		strSplitLocs += (splitLocs[i] + ", ");
            	}
            	LOG.info("Task:" + tip.getIdWithinJob() + " input at: " + strSplitLocs + " is assigned to  " + key);
            	numOfIndexLocalMaps ++;
            	LOG.info("Number of index locality: " + numOfIndexLocalMaps);
            }
  ...
  private void generateAndStoreTokens() throws IOException {
    ...
    Path hdfsFile = new Path(TOKEN_STORE_PATH_PREFIX+identifier.getJobId()+"_token");
